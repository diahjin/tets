...
## Nota plan gratuito Render

Esta versión no usa `preDeployCommand`, porque Render no lo admite en servicios gratuitos. Las migraciones se ejecutan en el `startCommand` antes de iniciar el servidor:

```bash
cd server && npx prisma migrate deploy && npm start
```

Si Render muestra el error de Blueprint anterior, reemplaza el archivo `render.yaml` por el incluido en este ZIP y vuelve a crear el Blueprint.
